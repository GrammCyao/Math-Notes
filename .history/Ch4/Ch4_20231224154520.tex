...
\begin{proof}
    Suppose $M$ is a nonempty smooth $m$-manifold, $N$ is a nonempty $n$-manifold, and $F : M \to N$ is a diffeomorphism. Choose any point $p \in M$, and let $(U, \varphi)$ and $(V, \psi)$ be smooth coordinate charts containing $p$ and $F(p)$, respectively. Then $\psi \circ F \circ \varphi^{-1}$ is a diffeom
\end{proof}
