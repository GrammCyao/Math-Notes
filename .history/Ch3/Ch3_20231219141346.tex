...
\begin{example}[Finite-dimensional vector spaces]
    Let $V$ be a finite-dimensional real vector space. Any norm on $V$ determines a topology, which is independent of the choice of norm. With this topology, $V$ is a topological $n$-manifold, and has a natural smooth structure defined as follows. Every basis $(v_j)_{1 \leq j \leq n}$ for $V$ defines a basis isomorphism $v : $
\end{example}
