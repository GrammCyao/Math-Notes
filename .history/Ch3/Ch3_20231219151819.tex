...
\begin{proof}
    ...
    Conversely, suppose that for every $p \in M$ there exists a neighborhood $U$ such that $F|_U$ is $C^\infty$. Then by definition, there exist charts $(V, \varphi)$ containing $p$ and $(W, \psi)$ containing $F(p)$ such that $U \cap F^{-1}(V)$ $\psi \circ F|_U \circ \varphi^{-1}$ is $C^\infty$. Since $V$ is an open subset of $U$, and $U$ is open in $M$. Thus $V$ is open subset of $M$
\end{proof}
