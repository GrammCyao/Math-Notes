...
\begin{proof}
    ...
    (iii) Let $p\in U$ be arbitrary. Let $(W,\varphi)$ be a chart on $U$ containing $p$. Then $(V,\varphi)$ is also a chart on $M$ containing $\iota(p) = p$. Then, $\varphi \circ \iota \circ \varphi^{-1} : \varphi(U) \to \varphi(U)$ is the identity map, which is smooth. Hence $\iota M is smooth.
