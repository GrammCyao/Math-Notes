...
\begin{proof}
    ...
    Conversely, suppose that for every $p \in M$ there exists a neighborhood $U$ such that $F|_U$ is $C^\infty$. Then by definition, there exist charts $(V, \varphi)$ containing $p$ and $(W, \psi)$ containing $F|_U(p)$ such that $V \cap F^{-1}|_U(W)$ is open in $U$ and $\psi \circ F|_U \circ \varphi^{-1}$ is $C^\infty$. Since $V$ is an open subset of $U$, and $U$ is open in $M$. Thus $V$ is open subset of $M$. Similarly, $V \cap F^{-1}|_U(W)$ is open in $M$. Thus $F$ is $C^\infty$.
\end{proof}
