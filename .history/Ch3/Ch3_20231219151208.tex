...
\begin{proof}
    ...
    (iv) Suppose that $F$ is smooth. Let $p \in M$ be arbitrary. Since $F$ is $C^\infty$, there are charts $(V, \varphi)$ containing $p$ and $(W, \psi)$ containing $F(p)$ such that $\psi \circ F \circ \varphi^{-1}$ is $C^\infty$. By Example \ref{ex:open submanifolds}, $V$ is an open submanifold. Thus for every $p \in U$, there exist $(U, )$
\end{proof}
