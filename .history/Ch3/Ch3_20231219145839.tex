...
\begin{proof}
    ...
    (iv) Suppose that $F$ is smooth. Let $U \subset M$ be an open open submanifold defined in Example \ref{ex:open submanifolds}. Let $p \in M$ be arbitrary. Since $F$ is $C^\infty$, there are charts $(V, \varphi)$ containing $p$ and $(W, \psi)$ containing $F(p)$
\end{proof}
