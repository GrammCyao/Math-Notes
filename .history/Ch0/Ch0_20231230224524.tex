...
\begin{proof}
    Suppose that $A \neq \emptyset$ and $A \in A$. Then we have $A \in \{A\}$. Since $A$ is a set, by the axiom of regularity, we only have $A \cap \{A\} = \emptyset$. But since A ∈ A and A ∈ {A}, this implies that A ∩ {A} = A, a contradiction. While if A is an empty set. By Axiom 3.3, A ∈/ A.
