...
\begin{proof}
    ...
    Note that we requires that $A \times B$ is non-empty, this equivalently to say that $A \neq \emptyset$ and $B \neq \emptyset$ from (v). If $A = \emptyset$, there is no elements such that $x \in A$ so that $x \in A \times B$. Then we can show that for $(x, y) \notin X \times Y$ and $(x, y) \in A \times B$.
\end{proof}
