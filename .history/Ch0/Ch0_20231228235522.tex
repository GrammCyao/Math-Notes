...
If $\mathcal{C}$ is a set which elements are sets, we call it a \emph{collection} instead of set. By the axiom of union, for every collection $\mathcal{C}$ there exists a set $U$ such that if $x \in X$ for some $X \in \mathcal{A}$
We can apply the axiom of specification to form the set
    \begin{align*}
        \{x \in \bigcup A : x \in S \text{ for some } S \in A\}.
    \end{align*}
