...
\begin{proof}
    Suppose that $A \in A$, then by the axiom of regularity, we only have $A \cap A = A = \emptyset$. Since empty set contains no elements, a contradiction. For the second assertion, suppose that both $A \in B$ and $B \in A$ are true, then by the axiom of 
\end{proof}
