...
\begin{proof}
    Suppose that $A \neq \emptyset$ and $A \in A$. Then we have $A \in \{A\}$. Since $A$ is  by the axiom of regularity, $A$ is either not a set, or is disjoint from {A}. So we know that A∩{A} = ∅. But since A ∈ A and A ∈ {A}, this implies that A ∩ {A} = A, a contradiction. While if A is an empty set. By Axiom 3.3, A ∈/ A.
