...
\begin{proof}
    ...
    Note that we requires $A \times B$ be non-empty, this equivalently to say that $A \neq \emptyset$ and $B \neq \emptyset$ from (v). If $A = \emptyset$, we must have $A \times B = \emptyset \subset X \times Y$ for arbitrary $B$. We can let $B \supsetneq Y$, then the premise still hold but the conclusion is not hold.
\end{proof}
