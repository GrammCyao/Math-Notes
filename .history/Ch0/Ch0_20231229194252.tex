...
\begin{proof}
    Suppose for sake of contradiction that $\emptyset \subset A$ is false. This means that there is an element of $\emptyset$ which doesn't lies in $A$. But it is impossible for that $\emptyset$ contains no elements, a contradiction. In particular, since the emptyset is a set, thus $$
\end{proof}
