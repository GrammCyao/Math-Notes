...
\begin{proof}
    (i) Suppose $a \in (A \cup B) \times X$, by definition, $a = (x, y)$ for some $x \in A \cup B$ and $y \in X$. Then we have $x \in A$ or $x \in B$. If $x \in A$, then $(x, y)$ for some $x \in A$ and $y \in X$, so that $$
\end{proof}
