...
If $\mathcal{C}$ is a set which elements are sets, we call it a \emph{collection} instead of set. By the axiom of union, for every collection $\mathcal{C}$ there exists a set $U$ such that if $x \in S$ for some $S \in \mathcal{A}$
