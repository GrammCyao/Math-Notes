...
\begin{definition}[Functions]
    Let $X, Y$ be sets. A \emph{function} from $X$ to $Y$ is a relation $f$ such that $\dom{f} = X$ and that for each $x \in X$ there is a unique $y \in Y$ such that $(x, y) \in f$. In other words, for each $x \in X$, if $(x, y) \in f$ and $(x, z) \in f$
\end{definition}
