...
\begin{proof}
    Suppose that $A$ is a non-empty set, and $A \in A$ By Axiom 3.4, we have A ∈ {A}. Then by Axiom 3.10, A is either not a set, or is disjoint from {A}. So we know that A∩{A} = ∅. But since A ∈ A and A ∈ {A}, this implies that A ∩ {A} = A, a contradiction. While if A is an empty set. By Axiom 3.3, A ∈/ A.
