...
\begin{definition}[Functions]
    Let $X, Y$ be sets, and let $P(x, y)$ be a property pertaining to an object $x \in X$ and an object $y \in Y$, such that for every $x \in X$, there is exactly one $y \in Y$ for which $P(x, y)$ is true. Then we define the \emph{function} $f : X \to Y$ defined by $P$ on the domain $X$ and range $Y$ 
\end{definition}
