...
\begin{proof}
    ...
    Note that we requires $A \times B$ be non-empty, this equivalently to say that both $A$ and $B$ are non-empty from (v). If $A = \emptyset$, we must have $A \times B = \emptyset \subset X \times Y$ for arbitrary $B$. We can let $B \supsetneq Y$, then the premise is true but the conclusion is false.
\end{proof}
