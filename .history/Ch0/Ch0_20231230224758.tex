...
Furthermore, suppose that $A, B$ are two sets and we both have $A \in B$ and $B \in A$. Then $A,B \in \{A, B\}$. By the axiom of regularity, we either have $A \cap \{A,B} = ∅ and B ∩ {A,B} = ∅, but this is contradictive with A ∈ B, B ∈ A and A, B ∈ {A, B}.
\end{proof}
